...
public class AddMeetingFragment extends Fragment implements InputTextChangeCallback {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            ...
            participantsLayout.setHint(getString(R.string.edit_input_participants_meeting));
            // Reset Error display
    ...
    private void updateDialogs() {
        ...
        if (dateFragment instanceof DatePickerMeetingDialog) {
            ((DatePickerMeetingDialog) dateFragment).setCallback(this);
        }

        // Hour Start Fragment
        Fragment hourStartFragment = getParentFragmentManager().findFragmentByTag(HOUR_BEGIN_DIALOG_TAG);
        if (hourStartFragment instanceof TimePickerMeetingDialog) {
            ((TimePickerMeetingDialog) hourStartFragment).setCallback(this);
        }

        // Hour End Fragment
        Fragment hourEndFragment = getParentFragmentManager().findFragmentByTag(HOUR_END_DIALOG_TAG);
        if (hourEndFragment instanceof TimePickerMeetingDialog) {
            ((TimePickerMeetingDialog) hourEndFragment).setCallback(this);
        }
        ...
        if (meetingRoomFragment instanceof MeetingRoomDialog) {
            ((MeetingRoomDialog) meetingRoomFragment).setCallback(this);
        }
    }
    ...
    private void onTextInputsEditTextListeners() {
        ...
        participantsInput.setOnClickListener((View view) -> {
                    ...
                    if (getActivity() instanceof MainActivityCallback) {
                        ...
                        ((MainActivityCallback) getActivity()).changeFragment(ListEmployeesFragment.newInstance(), ListMeetingsFragment.TAG);
                    }
                }
        );
    }
    ...
    private void handleOkBtnClickableStatus() {
        objectMeetingInput.addTextChangedListener(new TextWatcherTextInput(this));
        roomMeetingInput.addTextChangedListener(new TextWatcherTextInput(this));
        dateInput.addTextChangedListener(new TextWatcherTextInput(this));
        hourStartInput.addTextChangedListener(new TextWatcherTextInput(this));
        hourEndInput.addTextChangedListener(new TextWatcherTextInput(this));
        participantsInput.addTextChangedListener(new TextWatcherTextInput(this));
    }
    ...
    public void updateParticipantsTextInput() {
        ...
        if (meetingEmployees.size() > 0) {
            ...
            // Update Layout hint
            participantsLayout.setHint(getString(R.string.edit_input_participants_meeting) + "(" + nbMeetingEmployees + ")");
        } else {
            // Reset Layout hint
            participantsLayout.setHint(getString(R.string.edit_input_participants_meeting));
    ...
    public void onSetTime(String time, TimeType type) {
        if (type == TimeType.START) {
            hourStartInput.setText(time);
        } else {
            hourEndInput.setText(time);
        }
